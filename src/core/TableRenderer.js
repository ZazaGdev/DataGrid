...
import { createElement, addClass, removeClass, setAttributes } from '../utils/dom.js';
...
export class TableRenderer {
  ...
  _setupEventListeners() {
    this._eventBus.on(TableEvents.CELL_CHANGE, ({ rowId, columnName, newValue }) => {
  ...
  _createGroupHeaderRow(groupId, group, columns) {
    ...
    const toggleContent = createElement('div', { class: 'et-group-header-content' });
    
    const toggleIcon = createElement('span', {
      class: `et-group-toggle-icon ${isCollapsed ? 'et-collapsed' : 'et-expanded'}`
    });
  ...
  _createTotalsRow(totals, columns, groupId) {
    ...
    columns.forEach((column, index) => {
      ...
      if (totals[column.data] !== undefined) {
        td.textContent = this._formatDisplayValue(totals[column.data], column, {});
      }
  ...
  _toggleGroupVisibility(groupId, collapsed) {
    const rows = this._tbody.querySelectorAll(`[data-group="${groupId}"]:not(.et-row-group-header)`);
    const header = this._tbody.querySelector(`.et-row-group-header[data-group="${groupId}"]`);
